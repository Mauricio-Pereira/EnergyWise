...
public class ConsumoService {
    ...
    public ConsumoResponseDTO createConsumo(ConsumoRequestDTO consumoRequestDTO, Long idComunidade) {
        try {
            ...
            StoredProcedureQuery query = entityManager.createStoredProcedureQuery("INSERIR_CONSUMO");

            // Registrar os parâmetros de entrada
            query.registerStoredProcedureParameter("p_data_consumo", Timestamp.class, ParameterMode.IN);
            query.registerStoredProcedureParameter("p_energia_consumida", BigDecimal.class, ParameterMode.IN);
            query.registerStoredProcedureParameter("p_id_comunidade", Long.class, ParameterMode.IN);

            // **Registrar o parâmetro de saída**
            query.registerStoredProcedureParameter("p_id_consumo", Long.class, ParameterMode.OUT);

            // Definir os valores dos parâmetros de entrada
            query.setParameter("p_data_consumo", dataConsumo);
            query.setParameter("p_energia_consumida", consumoRequestDTO.getEnergiaConsumida());
            ...
            Long idConsumo = ((Number) query.getOutputParameterValue("p_id_consumo")).longValue();
            ...
            consumoResponseDTO.setEnergiaConsumida(consumoRequestDTO.getEnergiaConsumida());
            consumoResponseDTO.setNomeComunidade(comunidade.getNome());

            return consumoResponseDTO;

        } catch (Exception e) {
            throw new RuntimeException("Erro ao inserir consumo: " + e.getMessage(), e);
        }
    }
